...
module pwm_cdc #(
  ...
  // Regen field does not need
  // syncing, but assign it a value for completeness.
  ...
  always @(posedge clk_core_i, negedge rst_core_ni) begin
  ...
  // Though this may cause a single clock nondeteministic delay as
  // the buses wind though the CDC, this does not matter,
  // particularly since all channels will experience the same delay.

  assign clr_phase_cntr = ( common_sync_q != common_sync_out );


  genvar ii;
  for(ii = 0; ii < NOutputs; ii = ii + 1) begin : chan_cdc
    ...
    // Though it may be a bit overkill, we reset the internal blink
    // counters whenever any channel specific parameters change.
